...
import { resolveAdminPageChrome } from '@/lib/route-meta';
...
export function AdminAppShell({
  ...
  useEffect(() => {
    ...
    // One-time hydration from localStorage: the server cannot know the stored
    // preference, so the first client render reconciles it here.
  ...
  useEffect(() => {
    // Route changes are an external event: any open overlay must close.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setDrawerOpen(false);
    ...
    >
      ...
      >
        <div className="portal-sidebar__brand">
          <AdminBrand compact href="/dashboard" markOnly={collapsed} />
          <button
        ...
        <div className="portal-nav-backdrop" onMouseDown={() => closeDrawer()} role="presentation">
          ...
          >
            <header className="portal-nav-drawer__header">
              <div>
                <AdminBrand compact href="/dashboard" />
            ...
            <nav aria-label="Mobile administrator navigation" className="portal-nav-drawer__nav">
              {renderGroups(false, () => setDrawerOpen(false), 'mobile')}
            </nav>
