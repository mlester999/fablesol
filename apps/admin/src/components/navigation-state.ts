...
/**
 * Resolves the single active navigation href for a pathname. Longer hrefs win
 * so nested modules do not steal parent active state when paths overlap.
 */
...
export function groupAdminNavigationItems(
  ...
  for (const item of items) {
    const group = item.group?.trim() || 'Operations';
